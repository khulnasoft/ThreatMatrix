...
describe("test AppHeader component", () => {
  test("no auth header", async () => {
    ...
    expect(docsButton.closest("a").href).toBe(
      "https://khulnasoft.github.io/ThreatMatrix-docs//",
    );
    ...
    await waitFor(() => {
      expect(screen.getByText("Follow @threat_matrix")).toBeInTheDocument();
      expect(screen.getByText("Connect on Github")).toBeInTheDocument();
      expect(screen.getByText("ThreatMatrix on LinkedIn")).toBeInTheDocument();
