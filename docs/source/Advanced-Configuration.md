...
This page includes details about some advanced features that intel X provides which can be **optionally** configured by the administrator.
...
How to configure and enable LDAP on intel X?
...
How to configure and enable RADIUS authentication on intel X?
...
Instead of adding the variables to the environment file, you should just add them with the same name on the AWS Secrets Manager and intel X will fetch them transparently.
