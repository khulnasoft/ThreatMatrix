...
* `DJANGO_SECRET`: random 50 chars key, must be unique. If you do not provide one, IntelX will automatically set a secret key and use the same for each run. The key is generated by `initialize.sh` script.
...
IntelX provides basic configuration for:
* Nginx (`configuration/nginx/http.conf`)
* Uwsgi (`configuration/intelx.ini`)
...
```bash
$ cd <your_intelx_directory> # go into the project directory
...
- Bring only the postgres 12 container up `docker run -d --name intelx_postgres_12 -v intelx_postgres_data:/var/lib/postgresql/data/ --env-file env_file_postgres  library/postgres:12-alpine`
...
- Remove the postgres volume `docker volume rm intelx_postgres_data` <------------- remove old data, this is not exactly necessary because the new postgres has a different volume name
