...
<ul>
<li>The project uses public docker images that are available on <a href="https://hub.docker.com/repository/docker/khulnasoft/intelx">Docker Hub</a></li>
<li>IntelX is tested and supported to work in a Debian distro. More precisely we suggest using Ubuntu. Other Linux-based OS <i>should</i> work but that has not been tested much. It <i>may</i> also run on Windows, but that is not officially supported.</li>
<li>IntelX does not support ARM at the moment. We'll fix this with the next v6.0.5 release
<li>Before installing remember that you must comply with the <a href="https://github.com/certego/IntelX/blob/master/LICENSE">LICENSE</a> and the <a href="https://github.com/certego/IntelX/blob/master/.github/legal_notice.md">Legal Terms</a></li>
</ul>
</div>
...
## TL;DR
Obviously we strongly suggest reading through all the page to configure IntelX in the most appropriate way.

However, if you feel lazy, you could just install and test IntelX with the following steps. 
`docker` will be run with `sudo` if permissions/roles have not been set.

```bash
# clone the IntelX project repository
git clone https://github.com/khulnasoft/IntelX
cd IntelX/
...
# create a super user 
sudo docker exec -ti intelx_uwsgi python3 manage.py createsuperuser
...
<div class="admonition warning">
<p class="admonition-title">Warning</p>
The first time you start IntelX, a lot of database migrations are being applied. This requires some time. If you get 500 status code errors in the GUI, just wait few minutes and then refresh the page.
</div>
...
IntelX is composed of various different technologies, namely:
...
* `DB_HOST`, `DB_PORT`, `DB_USER`, `DB_PASSWORD`: PostgreSQL configuration (The DB credentals should match the ones in the `env_file_postgres`). If you like, you can configure the connection to an external PostgreSQL instance in the same variables. Then, to avoid to run PostgreSQL locally, please run IntelX with the option `--use-external-database`. Otherwise, `DB_HOST` must be `postgres` to have the app properly communicate with the PostgreSQL container.
* `DJANGO_SECRET`: random 50 chars key, must be unique. If you do not provide one, intel X will automatically set a secret key and use the same for each run. The key is generated by `initialize.sh` script.

  **Strongly recommended** variable to set:
* `INTELX_WEB_CLIENT_DOMAIN` (example: `localhost`/`mywebsite.com`): the web domain of your instance, this is used for generating links to analysis results.
...
* `POSTGRES_DB` (default: `intel_x_db`)
...
intel X provides basic configuration for:
* Nginx (`configuration/nginx/http.conf`)
* Uwsgi (`configuration/intel_x.ini`)
...
IntelX depends heavily on docker and docker compose so as to hide this complexity from the enduser the project
...
The CLI provides the primitives to correctly build, run or stop the containers for IntelX. Therefore,
<ul>
<li>It is possible to attach every optional docker container that IntelX has:
...
This token indicates the end of IntelX's options and the beginning of Docker options.
...
<div class="admonition hint">
<p class="admonition-title">Hint</p>
Starting from IntelX 4.0.0, with the startup script you can select which version of IntelX you want to run (<code>--version</code>).
...
### Users creation
You may want to run `docker exec -ti intelx_uwsgi python3 manage.py createsuperuser` after first run to create a superuser.
...
```bash
$ cd <your_intel_x_directory> # go into the project directory
$ git pull # pull new changes
$ ./start prod down # kill and destroy the currently running IntelX containers 
$ ./start prod up # restart the IntelX application
```
...
<div class="admonition warning">
<p class="admonition-title">Note</p>
After having upgraded IntelX, in case the application does not start and you get an error like this:
...
<div class="admonition warning">
<p class="admonition-title">Warning</p>
Major versions of IntelX are usually incompatible from one another.
...
IntelX v6 introduced some major changes regarding how the project is started.
...
* IntelX is now started with the new Bash `start` script that has the same options as the old Python `start.py` script but is more manageable and has decreased the overall project dependencies. The `start.py` script has now been removed. Please use the new `start` script instead.
* The default message broker is now Redis. We have replaced Rabbit-MQ for Redis to allow support for Websockets in the application:
  * This change is transparent if you use our `start` script to run IntelX. That would spawn a Redis instance instead of a Rabbit-MQ one locally.
...
Upgrading PostgreSQL is outside the scope of the IntelX project so we do not guarantee that everything will work as intended. 
...
- You have IntelX version 5.x.x up and running
...
- Bring only the postgres 12 container up `docker run -d --name intelx_postgres_12 -v intel_x_postgres_data:/var/lib/postgresql/data/ --env-file env_file_postgres  library/postgres:12-alpine`
- Dump the entire database. You need the user and the database that you configured during startup for this `docker exec -t intelx_postgres_12  pg_dump -U <POSTGRES_USER> -d <POSTGRES_DB> --no-owner > /tmp/dump_intelx.sql`
- Stop che container `docker container stop intelx_postgres_12`
- Remove the backup container `docker container rm intelx_postgres_12`
- Remove the postgres volume `docker volume rm intel_x_postgres_data` <------------- remove old data, this is not exactly necessary because the new postgres has a different volume name
- Start the intermediary postgres 16 container `docker run -d --name intelx_postgres_16 -v intelx_postgres_data:/var/lib/postgresql/data/ --env-file env_file_postgres  library/postgres:16-alpine`
- Add the data to the volume `cat /tmp/dump_intelx.sql | docker exec -i intelx_postgres_16 psql -U <POSTGRES_USER> -d <POSTGRES_DB>`
- Stop the intermediary container `docker container stop intelx_postgres_16`
- Remove the intermediary container `docker container rm intelx_postgres_16`
- Update IntelX to the latest version
...
IntelX v5 introduced some major changes regarding how the plugins and their related configuration are managed in the application.
Before upgrading, some important things should be checked by the administrator:
...
IntelX v4 introduced some major changes regarding the permission management, allowing an easier way to manage users and visibility. But that did break the previous available DB.
...
```
Please be aware that, while this can be an important effort to manage, the v4 IntelX provides an easier way to add, invite and manage users from the application itself. See [the Organization section](./Advanced-Usage.md#organizations-and-user-management).
...
If you are updating to >[v1.3.0](https://github.com/khulnasoft/IntelX/releases/tag/v1.3.0) from any prior version, you need to execute a helper script so that the old data present in the database doesn't break.

1. Follow the above updation steps, once the docker containers are up and running execute the following in a new terminal

    ```bash
    docker exec -ti intelx_uwsgi bash
    ```

    to get a shell session inside the IntelX's container.
