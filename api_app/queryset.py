# flake8: noqa
"""
This module defines custom query sets for various models used in the ThreatMatrix project.
Each query set provides additional methods for filtering, annotating, and manipulating
query results specific to the needs of the ThreatMatrix application.
"""
import datetime
import json
import uuid
        ...
        return self.alias(
            test_value=Case(
                When(
                    name__icontains="url",
                    then=Value(
                        "https://threatmatrix.khulnasoft.com", output_field=JSONField()
                    ),
                ),
                When(
